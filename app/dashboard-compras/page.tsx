...
const getCurrentUserEmail = () => {
  ...
  // Funciones para el filtro de meses
  const getAvailableMonths = () => {
    const months = new Set<string>();
    requisiciones.forEach(req => {
      if (req.fechaSolicitud) {
  ...
  // Filtrar requisiciones por meses seleccionados
  const getRequisicionesFiltradasPorMeses = () => {
    if (selectedMonths.length === 0) return requisiciones;
    
    return requisiciones.filter(req => {
      if (!req.fechaSolicitud) return false;
      const date = new Date(req.fechaSolicitud);
      const monthYear = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
  ...
  // Filtros combinados (incluyendo meses, empresas, estado y búsqueda)
  const filteredRequisiciones = requisiciones.filter((req) => {
    // Filtro por meses (primero para optimizar)
    if (selectedMonths.length > 0) {
      if (!req.fechaSolicitud) return false;
      const date = new Date(req.fechaSolicitud);
      const monthYear = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
      if (!selectedMonths.includes(monthYear)) return false;
    }
  ...
  // Stats (basadas en todos los filtros combinados)
  ...
  const enGestion = filteredRequisiciones.filter((r) => r.estado === 'en_gestion').length;
  const aprobadas = filteredRequisiciones.filter((r) => r.estado === 'aprobada').length;
  const rechazadas = filteredRequisiciones.filter((r) => r.estado === 'rechazada').length;
  const enCorreccion = filteredRequisiciones.filter((r) => r.estado === 'correccion').length;
  const cerradas = filteredRequisiciones.filter((r) => r.estado === 'completada').length;
  const pageSize = 50;
  ...
  return (
    <div className="dashboard-page">
      ...
      {/* FILTRO DE MESES - ESTADÍSTICAS */}
      <div className="month-filter-container">
        <div className="month-filter-header">
          <div className="month-filter-title">
            <Calendar className="filter-icon" />
            <span>Filtro por Meses</span>
          </div>
          <div className="month-filter-actions">
            <button 
              onClick={() => setShowMonthFilter(!showMonthFilter)}
              className="month-filter-toggle-btn"
            >
              {showMonthFilter ? <X size={16} /> : <Filter size={16} />}
              {showMonthFilter ? 'Cerrar' : 'Filtrar'}
            </button>
        ...
        {showMonthFilter && (
          <div className="month-filter-content">
            <div className="month-filter-description">
              Selecciona los meses para filtrar las estadísticas:
            </div>
            <div className="months-grid">
              {getAvailableMonths().map(month => (
                <button
                  ...
                  className={`month-chip ${selectedMonths.includes(month) ? 'selected' : ''}`}
                >
            ...
            {selectedMonths.length > 0 && (
              <div className="selected-months-summary">
                <span className="summary-text">
                  Mostrando estadísticas de: {selectedMonths.map(m => formatMonthDisplay(m)).join(', ')}
                </span>
              </div>
            )}
          </div>
      ...
      <div className="stats-grid">
        <div className="stat-card">
          ...
          <p className="stat-description">Requisiciones totales</p>
        </div>
        ...
        <div className="stat-card">
          <div className="stat-card-header">
            <span className="stat-card-title">EN GESTIÓN</span>
            <AlertTriangle className="stat-icon" />
          </div>
          <div className="stat-value yellow">{enGestion}</div>
          <p className="stat-description">
            {enGestion > 0 ? `${enGestion} siendo procesadas` : 'Sin requisiciones en gestión'}
          </p>
        </div>
        ...
        <div className="stat-card">
          ...
          <p className="stat-description">Requisiciones completadas</p>
        </div>
      </div>

      {/* CHARTS */}
      <RequisitionCharts selectedMonths={selectedMonths} />
      ...
      <div className="mt-10">
        ...
        {/* FILTRO DE MESES */}
        <div className="month-filter-container mb-6">
          <div className="month-filter-header">
            <div className="month-filter-title">
              <Calendar className="filter-icon" />
              <span>Filtro por Meses</span>
